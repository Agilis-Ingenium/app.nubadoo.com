...
function DeleteActivity(id, index, items) {
  if (
    confirm("Are you sure that you really want to delete the activity entry?")
  ) {
    axios
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-6 py-4">
            ...
            {{ item.durationMinutes != null ? "min" : "" }}
          </td>
          <td class="px-6 py-4">
            <WidgetGauge
              :percentage="intensityPercentage(item.workoutIntensity)"
            />
          </td>
          <td class="px-6 py-4">
          ...
          <td class="px-6 py-4">
            <a
              href="javascript:;"
              v-on:click="DeleteActivity(item.activityId, index, this.items)"
            >
              <WidegtDeleteButton />
            </a>
          </td>
